...
module.exports = function(currentTemplate, aliasStackTemplates, currentAliasStackTemplate) {
	...
	_.forOwn(cwEvents, (cwEvent, name) => {
		...
		if (_.isString(cwEvent.DependsOn)) cwEvent.DependsOn = [cwEvent.DependsOn];
		_.forEach(targetRefs, ref => {
			const functionName = _.replace(ref.ref, /LambdaFunction$/, '');
			if (functionName !== ref.ref) {
				// Lambda function
				...
				const res = stageStack.Resources[ref.ref];
				if (_.get(res, 'Type') !== 'AWS::IAM::Role') { return; }

				// We reference a role => move role to alias stack
				delete stageStack.Resources[ref.ref];
				aliasStack.Resources[ref.ref] = res;
			}
		});
