...
const internal = {
	...
	createStageResource(restApiRef, deploymentName) {
		...
		const serviceLevelConfig = _.cloneDeep(_.get(this.serverless.service, 'custom.aliasStage', _.get(this.serverless.service, 'provider.aliasStage', {})));
...
module.exports = function(currentTemplate, aliasStackTemplates, currentAliasStackTemplate) {
	...
	if (exposeApi) {
		...
		_.forOwn(apiMethods, (method, name) => {
			...
			_.remove(method.DependsOn, dep => dep.endsWith('LambdaPermissionApiGateway'));
