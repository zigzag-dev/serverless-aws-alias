...
module.exports = {
	...
	aliasRemoveAliasStack(currentTemplate, aliasStackTemplates, currentAliasStackTemplate) {
		...
		let stackId = null;
		return this._provider.request('CloudFormation', 'describeStacks', {StackName: stackName}, this._options.stage, this.options.region)
			.then(cfData => {
				if (cfData.Stacks.length === 0) {
					throw new Error(`Stack ${stackName} does not exist`);
				}
				stackId = cfData.Stacks[0].StackId;
				return stackId;
			})
			.then(() => this._provider.request('CloudFormation', 'deleteStack', {StackName: stackName}, this._options.stage, this.options.region))
			.then(() => {
				// monitorStack wants a StackId member
				return this.monitorStack('removal', {StackId: stackId});
			})
			.then(() => {
				return BbPromise.resolve([currentTemplate, aliasStackTemplates, currentAliasStackTemplate]);
			})
